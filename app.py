...
# Guardar mensajes en memoria temporal
mensajes_guardados = []

@app.route("/")
def index():
    return "Webhook CMD activo (sin MongoDB)", 200
...
def webhook():
    if request.method == "GET":
        ...
        if data:
            mensajes_guardados.append({"data": data, "timestamp": datetime.utcnow().isoformat()})
            try:
...
def obtener_mensajes():
    return jsonify(mensajes_guardados), 200
